...
public class NatalChartHandler implements RequestHandler<Object, NatalChartResponse> {
   ...
   public NatalChartResponse handleRequest(Object event, Context context) {
      ...
      if(!(event instanceof NatalChartRequest)) {
         error = "Received object that is not instance of NatalChartRequest. Object type is " + event.getClass().getName();
         if(event instanceof LinkedHashMap) {
            Map requestMap = (LinkedHashMap)event;
            //requestMap.forEach((k, v) -> {
              // logger.log("key: " + k + "; value: " + v + "; value type: " + v.getClass().getName());
               //if(k.equals("body")) {
      ...
      //NatalChartRequest request = (NatalChartRequest) event;
      if(request.isEmpty()) {
         error = "NatalChartRequest object is empty";
         logger.log(error);
         response.setError(error);
         return response;
      }
      ...
      logger.log("Real Input EVENT class: " + event.getClass());
      logger.log("Real Input EVENT: " + request);
      ...
      response = (new NatalChartResponse()).setCusps(cusps).setPlanets(thePlanets);
